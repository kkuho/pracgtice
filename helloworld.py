...
# mid = datetime.datetime(now.year, now.month, now.day) + datetime.timedelta(minutes=5) # 자정 시간을 구하는 함수 식
mid = datetime.datetime.now() + datetime.timedelta(seconds=10) # 자정 시간을 구하는 함수 식
print(now)
print(mid)


ticker_input = "krw-eth"
...
while True:
    ...
    if mid < now < mid + datetime.timedelta(seconds=5): # 정각에서 10초 내에 있을 때 자정으로 간주함
        print("정각입니다!!")
