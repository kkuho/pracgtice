...
def getgapsize(askprice):
    return round(askprice * 0.02, min_unit(askprice))
    ...
    try :
        ...
        if len(myval) < 2 : # 보유한 coin이 있는지 확인. 2보다 작으면 보유한 coin이 없는 것으로 보고, 매수 로직 가동
            ...
            if (krw >=5000):
                ...
                trade = buy_crypto_currency(ticker_input, baseprice)
                buyflag = False
            ...
            if pyupbit.get_current_price(avaTicker) > float(myval[1]['avg_buy_price']) * 1.015 :
                    ...
                    for item in record:
                        cancel = upbit.cancel_order(item)
                        print(cancel)
            ...
            for item in record[:]:
                ...
                if buyflag == False and upbit.get_order(item)['state'] == 'done':
                    avaTicker = 'KRW-' + myval[1]['currency']
                    sellsubprice = float(upbit.get_order(item)['price']) + gaptick
