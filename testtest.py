...
def check_record(): # 미체결 주문이 있는지 확인하고 있으면, 해당 uuid를 record에 저장
    ...
    if len(myval) > 1:
        for ia in range(1, len(myval)):
            preTicker = 'KRW-' + myval[ia]['currency']
...
while True:
    ...
    if len(myval) < 2 : # 보유한 coin이 있는지 확인. 2보다 작으면 보유한 coin이 없는 것으로 보고, 매수 로직 가동
        ...
        for idx, ticker_input in enumerate(sortlist[0:13]) :
            ...
            if buyflag and buying_flag1 and buying_flag2 and (current_price <= yesterday_close) and (krw >=5000) :
                ...
                # baseprice1 = upbit.get_balance()//20 * 0.995 # 예수금의 10%를 baseprice로 매수 진행
                
                trade = buy_crypto_currency(ticker_input[0], baseprice)
                buyflag = False
                
                gaptick1 = getgapsize(askprice1)
                ...
                for i in range(1,9): 
                    subgetprice = 0
                    subgetprice = round(askprice1 - gaptick1*i)
                ...
                print(curtime,"| ticker: " ,ticker_input[0]," | target_flag: ", buying_flag1, " | ma5_flag: " ,buying_flag2," | 현재가 : " ,current_price," | 10분전 : ", yesterday_close)
            time.sleep(0.2)
        

    elif 1 < len(myval) < 3: # 보유하고 있는 코인이 한 개일 경우 
        ...
        for idx, ticker_input in enumerate(sortlist[0:10]) :
            ...
            if buyflag and buying_flag1 and buying_flag2 and (current_price <= yesterday_close) and (krw >=5000) :
                ...
                write_balance(curtime, krw)
                # baseprice2 = upbit.get_balance()//20 * 0.995 # 예수금의 5%를 baseprice로 매수 진행
                trade = buy_crypto_currency(ticker_input[0], baseprice)
                buyflag = False
                ...
                for i in range(1,9): 
                    subgetprice = 0
                    subgetprice = round(askprice2 - gaptick2*i)
    ...
    if 1 < len(myval): # 2개의 종목을 가지고 수익율 체크 후 매도, 거미줄 치기 연속
        ...
        for item in record[:]:
            ...
            if  upbit.get_order(item[0])['side'] == 'bid' and upbit.get_order(item[0])['state'] == 'done':
                avaTicker = item[2]
                gaptick = getgapsize(pyupbit.get_current_price(avaTicker))
                ...
                print(ret)
                record.append([ret['uuid'], datetime.datetime.now(), avaTicker, pyupbit.get_current_price(avaTicker), baseprice, gaptick, buysubprice, '추가 거미줄매수'])
                record.remove(item)
                delete_trade()
                write_record(record)
            time.sleep(0.2)
